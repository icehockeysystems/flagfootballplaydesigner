...
// Update paths to point to public_html directory
const publicHtmlPath = path.join(__dirname, '..', 'public_html');
const playsDirectory = path.join(publicHtmlPath, 'plays');
...
// Add this specific route for the plays directory
app.use('/plays', express.static(path.join(publicHtmlPath, 'plays')));

// Make sure this comes AFTER the CORS middleware and BEFORE the POST endpoint
app.get('/plays/:filename', (req, res) => {
  const filename = req.params.filename;
  res.sendFile(path.join(playsDirectory, filename));
});
